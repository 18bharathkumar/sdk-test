import {
  ...
  const YOUR_WALLET = "DZyffHk28yLcV3PH7t9U88stZkKZLFgx94K8KkfzByXN";
  ...
  const onCreatePool = async () => {
      ...
      const binStep = BIN_STEP_CONFIGS?.[3]?.binStep ?? 0; // Safely access binStep, fallback to 0 if undefined
      const ratePrice = 1; // Example rate price, you can set it based on your requirements
      let payer: PublicKey;
      try {
        payer = new PublicKey(YOUR_WALLET);
      } catch (e) {
        throw new Error("Invalid wallet address provided for payer.");
      }
